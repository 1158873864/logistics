...
public class VersionServiceImpl implements VersionService {
    ...
    @Override
    public Version save() {
        return versionRepository.save(new Version((long)2,"2","",""));
    }

    @Override
    public Version get2() {
        Optional<Version> optionalVersion=versionRepository.findById((long)2);
        if(optionalVersion.isPresent()){
            return optionalVersion.get();
        }
        else{
    ...
    @Override
    public void set2(String number, String path,String info) {
        Optional<Version> optionalVersion=versionRepository.findById((long)2);
        Version version=null;
        if(optionalVersion.isPresent()){
            version=optionalVersion.get();
