...
public class TopicServiceImpl implements TopicService {
    ...
    @Override
    public Page<Topic> find(String query, Pageable pageable) {
        List<Topic> topics=topicRepository.findAll();
        List<Topic> sortedList=new ArrayList<>();
        for(int i=0;i<topics.size();i++){
            Topic topic=topics.get(i);
            if(topic.getUserInfo().getMobilePhone().indexOf(query)!=(-1)||topic.getContent().indexOf(query)!=(-1)||topic.getTitle().indexOf(query)!=(-1)||topic.getCommentCount().toString().indexOf(query)!=(-1)||topic.getFabulousCount().toString().indexOf(query)!=(-1)||topic.getForwardCount().toString().indexOf(query)!=(-1)||topic.getPublished().toString().indexOf(query)!=(-1)||topic.getStatus().toString().indexOf(query)!=(-1)||topic.getTitle().indexOf(query)!=(-1)||String.valueOf(topic.getId()).indexOf(query)!=(-1)){
                sortedList.add(topic);
            }
        }
