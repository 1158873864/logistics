...
public class LogisticsDdnPicServiceImpl implements LogisticsDdnPicService {
    ...
    @Override
    public Page<LogisticsDdnPic> find(String query, Pageable pageable){
        List<LogisticsDdnPic> logisticsDdnPics=logisticsDdnPicRepository.findAll();
        List<LogisticsDdnPic> sortedList=new ArrayList<>();
        System.out.println("logisticsddn:"+logisticsDdnPics.size());
        for(int i=0;i<logisticsDdnPics.size();i++){
            LogisticsDdnPic logisticsDdnPic=logisticsDdnPics.get(i);
            if(logisticsDdnPic.getRemark().indexOf(query)!=(-1)||logisticsDdnPic.getTitle().indexOf(query)!=(-1)||logisticsDdnPic.getLogisticsDdn().getTitle().indexOf(query)!=(-1)||logisticsDdnPic.getStatus().toString().indexOf(query)!=(-1)||String.valueOf(logisticsDdnPic.getId()).indexOf(query)!=(-1)){
                sortedList.add(logisticsDdnPic);
            }
        }

//        int pageNum=pageable.getPageNumber();
//        int pageSize=pageable.getPageSize();
//        int max=(sortedList.size()<((pageNum+1)*pageSize))?sortedList.size():(pageNum+1)*pageSize;
